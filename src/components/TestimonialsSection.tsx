...
export default function TestimonialsSection() {
  ...
  return <section className="bg-muted py-8 sm:py-12 lg:py-16">
      <div className="container px-4">
        ...
        <div className="relative max-w-4xl mx-auto">
          <div className="relative min-h-[320px] sm:min-h-[350px] md:min-h-[280px]">
            {testimonials.map((testimonial, index) => <div key={testimonial.id} className={cn("absolute inset-0 glass-card p-4 sm:p-6 md:p-8 transition-all duration-500", activeIndex === index ? "opacity-100 translate-x-0 z-10" : index < activeIndex ? "opacity-0 -translate-x-full z-0" : "opacity-0 translate-x-full z-0")}>
                <div className="flex flex-col md:flex-row gap-4 sm:gap-6 h-full">
                  <div className="flex flex-col items-center md:items-start shrink-0">
                    <div className="rounded-full overflow-hidden w-12 h-12 sm:w-16 sm:h-16 md:w-20 md:h-20 mb-2 sm:mb-3 md:mb-4 border-2 border-primary">
                    ...
                    <h4 className="text-sm sm:text-base md:text-lg font-semibold text-center md:text-left">{testimonial.name}</h4>
          ...
          <div className="flex justify-between items-center mt-6 sm:mt-8">
            <button onClick={prevTestimonial} className="p-1.5 sm:p-2 rounded-full bg-card hover:bg-muted border border-border transition-colors" disabled={isAnimating}>
            ...
            }} className={`w-2 h-2 sm:w-3 sm:h-3 rounded-full transition-all ${activeIndex === index ? "bg-primary w-4 sm:w-6" : "bg-muted-foreground/30 hover:bg-muted-foreground/50"}`} aria-label={`Go to testimonial ${index + 1}`} />)}
            </div>
            
            <button onClick={nextTestimonial} className="p-1.5 sm:p-2 rounded-full bg-card hover:bg-muted border border-border transition-colors" disabled={isAnimating}>
