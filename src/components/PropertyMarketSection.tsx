...
const PropertyRow: React.FC<PropertyRowProps> = ({
  ...
  return <motion.div className="relative p-3 rounded-xl bg-white/95 backdrop-blur-sm border border-white/20 shadow-lg hover:shadow-xl transition-all duration-300 group z-10 h-[160px] w-full flex flex-col justify-between" whileHover={{
    scale: 1.02,
    y: -2
  }} transition={{
    duration: 0.2
      ...
      <div className="flex items-start gap-1.5 flex-1 min-h-0">
        ...
        <div className="flex-1 min-w-0">
          <span className="text-2xs font-semibold text-gray-900 block mb-1 leading-tight break-words">{label}</span>
          <div className="flex flex-col">
            <span className="text-2xs font-bold text-gray-900 leading-tight break-words">
              ${price.toLocaleString()}
            </span>
            <span className="text-2xs text-gray-600 leading-tight font-medium">/m²</span>
          </div>
        </div>
      </div>
      
      <div className="flex justify-center mt-2">
        <div className={`flex items-center gap-1 px-2 py-1 rounded-full text-2xs font-bold shadow-sm ${isPositive ? 'bg-gradient-to-r from-green-500 to-emerald-500 text-white' : 'bg-gradient-to-r from-red-500 to-rose-500 text-white'}`}>
  ...
  return <motion.div className="relative p-8 rounded-3xl bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl hover:shadow-3xl transition-all duration-500 group overflow-hidden cursor-pointer z-0" whileHover={{
    ...
    ease: "easeOut"
      ...
      <div className="relative z-10">
        ...
        <motion.div className="grid grid-cols-1 gap-4" animate={isExpanded ? {
        opacity: 1,
        y: 0
      } : {
        opacity: 1,
