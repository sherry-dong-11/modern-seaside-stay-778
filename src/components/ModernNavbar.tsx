    ...
    >
      <nav className="container flex justify-between items-center">
        ...
        >
          ...
          <motion.li 
            className="relative"
            onHoverStart={() => setNewHomeDropdownOpen(true)}
            onHoverEnd={() => setNewHomeDropdownOpen(false)}
          >
            <motion.button 
              className={cn(
                "font-medium transition-all duration-300 flex items-center gap-1 py-2 px-3 rounded-lg relative overflow-hidden",
                scrolled ? "text-foreground hover:text-primary" : "text-white hover:text-orange-300"
              )}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
            >
              <motion.div
                className="absolute inset-0 bg-gradient-to-r from-orange-500/10 to-pink-500/10 rounded-lg opacity-0"
                whileHover={{ opacity: 1 }}
                transition={{ duration: 0.3 }}
              />
              <span className="relative z-10">New Home</span>
              <motion.div
                animate={{ rotate: newHomeDropdownOpen ? 180 : 0 }}
                transition={{ duration: 0.3 }}
              >
            ...
            <AnimatePresence>
              {newHomeDropdownOpen && (
                ...
                >
                  <div className="py-2">
                    {newHomeDropdownItems.map((item, index) => (
                      <motion.div
                        key={item.name}
                      ...
                      >
                        <Link 
                          to={item.path} 
                          className="block px-6 py-3 text-sm font-medium hover:bg-gradient-to-r hover:from-orange-500/10 hover:to-pink-500/10 transition-all duration-300 relative group"
                        >
                          <motion.div
                            className="absolute left-0 top-0 bottom-0 w-1 bg-gradient-to-b from-orange-500 to-pink-500 scale-y-0 group-hover:scale-y-100 origin-bottom"
                            transition={{ duration: 0.3 }}
                          />
          ...
          {navLinks.map((link, index) => (
            ...
            >
              <Link 
                to={link.path} 
                className={cn(
                  "font-medium transition-all duration-300 py-2 px-3 rounded-lg relative overflow-hidden group",
                  scrolled ? "text-foreground hover:text-primary" : "text-white hover:text-orange-300"
                )}
              >
                <motion.div
                  className="absolute inset-0 bg-gradient-to-r from-orange-500/10 to-pink-500/10 rounded-lg opacity-0 group-hover:opacity-100"
                  transition={{ duration: 0.3 }}
                />
          ...
          <motion.li 
            className="relative"
            onHoverStart={() => setProfessionalsDropdownOpen(true)}
            onHoverEnd={() => setProfessionalsDropdownOpen(false)}
            initial={{ opacity: 0, y: -20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            ...
            >
              ...
              <motion.div
                animate={{ rotate: professionalsDropdownOpen ? 180 : 0 }}
                transition={{ duration: 0.3 }}
              >
                <ChevronDown className="h-4 w-4" />
              </motion.div>
            ...
            <AnimatePresence>
              {professionalsDropdownOpen && (
                ...
                >
                  <div className="py-2">
                    {professionalsDropdownItems.map((item, index) => (
                      <motion.div
                        key={item.name}
                        initial={{ opacity: 0, x: -20 }}
                        animate={{ opacity: 1, x: 0 }}
