...
const categories = [
  ...
  { id: "press", label: "Press Releases", active: false },
  { id: "stories", label: "Customer Stories", active: false },
];
        ...
        <div className="container mx-auto px-4 py-12 max-w-6xl">
          ...
          <div className="mb-12">
            ...
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  className="pl-10 pr-4 py-2 w-64 rounded-full border-slate-200 focus:border-primary"
                />
              </div>
            </div>
            ...
            <h1 className="text-4xl md:text-5xl font-bold text-slate-900 mb-4">
              {activeCategory === "all" ? "Latest News and Blogs" : 
               activeCategory === "blog" ? "Blog" :
               activeCategory === "press" ? "Press Releases" : "Customer Stories"}
            </h1>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredPosts.map((post) => (
              <Card 
                key={post.id} 
                className={`group cursor-pointer overflow-hidden border-0 shadow-lg hover:shadow-xl transition-all duration-300 ${post.bgColor}`}
              >
                ...
                <div className="p-6">
                  <h3 className={`text-lg font-semibold mb-3 leading-tight group-hover:text-primary transition-colors ${
                    post.isDark ? "text-white" : "text-slate-900"
                  }`}>
                    {post.title}
                  </h3>
          ...
          {filteredPosts.length > 6 && (
            <div className="text-center mt-12">
              <Button 
                variant="outline" 
                size="lg"
                className="rounded-full px-8 py-3 border-slate-200 hover:bg-slate-50"
              >
                Load More Articles
              </Button>
            </div>
